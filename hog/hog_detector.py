...
#directory_to_cycle = "D:/ComputerVisionAssignment/pedestrain/INRIAPerson/Test/pos/";
...
class HOGdetector():
    ...
    def __init__(self):#, PATH_Train
        ...
        try:
            self.svm = cv2.ml.SVM_load(params.HOG_SVM_PATH)
        except:
            ...
            if step > 0:
                ...
                for (x, y, window) in sliding_window(resized, window_size, step_size=step):
                    ...
                    if img_data.hog_descriptor is not None:
                        ...
                        elif result[0] == params.DATA_CLASS_NAMES["car"]:
                            # store rect as (x1, y1) (x2,y2) pair
                            rect = np.float32([x, y, x + window_size[0], y + window_size[1]])
                            # if we want to see progress show each detection, at each scale
                        ...
                        elif result[0] == params.DATA_CLASS_NAMES["motorbike"]:
                            # store rect as (x1, y1) (x2,y2) pair
                            rect = np.float32([x, y, x + window_size[0], y + window_size[1]])
                            # if we want to see progress show each detection, at each scale
    ...
    def detectSelective(self,frame):
        "for selective search, pretty bad tbf"
        ...
        totals=self.ss.detect(frame)

        if totals!=None:
            for each in totals:
                img_data = ImageData(output_img[each[0]:each[1],each[2]:each[3]])
                img_data.compute_hog_descriptor();
                ...
                if img_data.hog_descriptor is not None:
                    ...
                    if result[0] == params.DATA_CLASS_NAMES["pedestrain"]:
                        ...
                        frame[each[0]:each[1],each[2]:each[3]]
                        # store rect as (x1, y1) (x2,y2) pair
                        rect = np.float32([each[2], each[0], each[3], each[1]])
                        cv2.rectangle(output_img, (rect[0], rect[1]), (rect[2], rect[3]), (0, 0, 255), 2)
                        #
                        # # if we want to see progress show each detection, at each scale
                        #
